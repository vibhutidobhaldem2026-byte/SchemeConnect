...
import { matchableSchemes, catalogMeta, catalogAgeDays } from '../catalog.js';
...
router.get('/', async (req, res) => {
  const inst = await institute(req);
  const batches = (await store.getBatches(inst.id)).filter((b) => b.status === 'imported');
  ...
  const matchedStudents = batches.reduce(
    (n, b) => n + (b.students ?? []).filter((s) => s.matchCount > 0).length, 0);
          ...
          ${raw(batches.length ? html`
            <div class="table-wrap">
              <table>
                ...
                ${raw(batches.map((b) => {
                  const n = b.students?.length ?? 0;
                  const m = (b.students ?? []).filter((s) => s.matchCount > 0).length;
                  return html`<tr class="clickable" onclick="location.href='/institute/students?batch=${b.id}'">
...
router.post('/upload', async (req, res) => {
  ...
  const inst = await institute(req);
  const draft = await store.saveBatch({
    instituteId: inst.id,
...
router.post('/preview/:id', async (req, res, next) => {
  ...
  await store.updateBatch(batch.id, {
    status: 'imported',
    rows: [], // raw rows are no longer needed once mapped
    mapping,
    students,
  });
...
router.get('/students', async (req, res) => {
  ...
  let students = batches
    .filter((b) => !batchFilter || b.id === batchFilter)
    .flatMap((b) => (b.students ?? []).map((s) => ({ ...s, batchLabel: b.label, batchId: b.id })));
      ...
      <div class="app-shell">
        ...
        <main class="main-area">
          ...
          ${raw(students.length ? html`
            <div class="table-wrap">
              <table>
                <tr><th>Name</th><th>Student ID</th><th>Class</th><th>State</th><th>Category</th><th>Matches</th><th>Status</th></tr>
                ${raw(students.slice(0, 300).map((s) => html`
                  <tr class="clickable" onclick="location.href='/institute/students/${s.batchId}/${encodeURIComponent(s.studentId || s.name)}'">
                    <td class="b">${s.name}</td>
                    <td>${s.studentId || '—'}</td>
                    <td>${s.profile?.courseLevel || '—'}</td>
                    <td>${s.profile?.state || '—'}</td>
                    <td>${s.profile?.category || '—'}</td>
                    <td>${s.matchCount ?? 0}</td>
                    <td><span class="pill ${s.matchCount > 0 ? 'pill-active' : 'pill-none'}">
                      ${s.matchCount > 0 ? 'Matched' : 'No match yet'}</span></td>
                  </tr>`).join(''))}
              </table>
            </div>` : emptyState('👥', 'No students yet',
...
router.get('/students/:batchId/:studentId', async (req, res, next) => {
  const batch = await store.getBatch(req.params.batchId);
  if (!batch) return next();
  const key = decodeURIComponent(req.params.studentId);
  const student = (batch.students ?? []).find((s) => (s.studentId || s.name) === key);
        ...
        <main class="main-area narrow">
          ...
          <div class="profile-header">
            ...
            <div>
              ...
              <div class="greeting-sub" style="margin-bottom:0;">${student.studentId || '—'} · ${batch.label}</div>
            </div>
          </div>
