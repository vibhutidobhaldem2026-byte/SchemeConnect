...
router.get('/', async (req, res) => {
  ...
  const teasers = schemes.filter((s) => s.detailLevel === 'full').slice(0, 3);
  const fallback = teasers.length ? teasers : schemes.slice(0, 3);
...
router.get('/schemes', async (req, res) => {
  ...
  if (level === 'central' || level === 'state') schemes = schemes.filter((s) => s.level === level);
  if (level === 'matchable') schemes = schemes.filter((s) => s.detailLevel === 'full');

  const shown = schemes.slice(0, 200);
    ...
    body: html`
      <div class="app-shell">
        ...
        <main class="main-area">
          ...
          <div class="section-label">${schemes.length} scheme${schemes.length === 1 ? '' : 's'}${q ? ` matching “${q}”` : ''}</div>
          ...
          ${raw(schemes.length > shown.length
            ? html`<p class="foot-note">Showing the first ${shown.length} of ${schemes.length}. Narrow your search to see more.</p>`
            : '')}
        </main>
      </div>`,
  }));
