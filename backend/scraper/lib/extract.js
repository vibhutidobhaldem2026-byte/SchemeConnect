...
export function extractCourseLevels(text) {
  ...
  let evidence = '';
  /**
   * Degree abbreviations are matched CASE-SENSITIVELY, and the dotted and
   * undotted forms are separate patterns.
   *
   * The previous single case-insensitive pattern for "B.A / B.Sc / B.E" made
   * both the dot and the space optional, so it matched the ordinary English
   * word "be" — and the Postgraduate one matched "me" and "ma". One sentence
   * about grants to "eligible" agencies that "will be given" was therefore read
   * as "this scheme is for Undergraduates", which is how a senior-citizens
   * scheme ended up being recommended to students.
   *
   * A degree is written "B.A", "BA" or "B Tech" in these documents — always
   * capitalised. Lowercase "be" is a verb.
   */
  const rules = [
    ...
    [/\bdiploma\b|\bpolytechnic\b/i, 'Diploma'],
    [/\bITI\b/, 'Diploma'],
    [/\bunder[\s-]?graduate\b|\bbachelor(?:'?s)?\b|\bgraduation\b/i, 'Undergraduate'],
    [/\bB\.\s?(?:A|Sc|Com|Tech|Ed|E)\b|\b(?:BA|BSc|BCom|BTech|BEd|BE|UG)\b/, 'Undergraduate'],
    [/\bpost[\s-]?graduate\b|\bmaster(?:'?s)?\b/i, 'Postgraduate'],
    [/\bM\.\s?(?:A|Sc|Com|Tech|Ed|E)\b|\b(?:MA|MSc|MCom|MTech|MEd|PG)\b/, 'Postgraduate'],
    [/\bdoctoral\b|\bresearch\s+scholar\b/i, 'PhD'],
    [/\bPh\.?\s?D\b|\bM\.?\s?Phil\b/, 'PhD'],
  ];
  for (const s of sentences(text)) {
    // "eligible" alone is far too weak — it appears in grant and procurement
    // prose throughout these documents. The sentence has to be about someone
    // studying.
    if (!/\bstud(?:y|ying|ent|ents)\b|\bpursu(?:e|ing)\b|\benrol(?:led|ment)\b|\badmitted\b|\bcourse\b|\bclass\b|\bschool\b|\bcollege\b|\buniversity\b|\bscholar(?:ship)?\b/i.test(s)) continue;
